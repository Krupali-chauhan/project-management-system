...
function SOWDetail() {
  ...
  const updateStatusMutation = useMutation({
    ...
    onSuccess: (response) => {
      ...
      if (updated.status === "approved") {
        alert("SOW Approved! Waiting for Project Manager assignment.");
        navigate("/client/view-sow");
      } else if (updated.status === "rejected") {
        alert("SOW Rejected. You can now edit and resubmit.");
        navigate(`/client/create-project/${id}`);
      }
    },
    onError: (err) => {
  ...
  if (isError || !project) return (
    <Box sx={{ maxWidth: 900, mx: "auto", mt: 8 }}>
      <Alert severity="error">Project not found or error loading data.</Alert>
      <Button
        startIcon={<ArrowBackIcon />}
        onClick={() => navigate("/client/view-sow")}
        sx={{ mt: 3 }}
  ...
  const isApproved  = project.status === "approved";
  const isRejected  = project.status === "rejected";
  const isAssigned  = project.status === "assigned" || project.status === "in_progress";

  return (
    <Box sx={{ maxWidth: 1000, mx: "auto", mt: 6, px: { xs: 2, md: 0 } }}>
      {/* Header */}
      <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 4 }}>
        <Button startIcon={<ArrowBackIcon />} onClick={() => navigate("/client/view-sow")}>
          Back to My SOWs
        </Button>
        <Tooltip title="Print SOW">
      ...
      {/* Status Messages */}
      {isApproved && !project.projectManagerId && (
        <Alert severity="success" icon={<CheckCircleIcon />} sx={{ mb: 4 }}>
          SOW Approved — awaiting Project Manager assignment
        </Alert>
      )}

      {isAssigned && (
        <Alert severity="info" icon={<CheckCircleIcon />} sx={{ mb: 4 }}>
          Project is now active with assigned Project Manager
        </Alert>
      )}

      {isRejected && (
        <Alert severity="error" icon={<CancelIcon />} sx={{ mb: 4 }}>
          SOW Rejected — please edit and resubmit
        </Alert>
      )}
      ...
      <Paper elevation={6} sx={{ borderRadius: 3, overflow: "hidden" }}>
        {/* Header Gradient */}
        <Box sx={{ bgcolor: "primary.main", color: "white", p: 4, pb: 6 }}>
          <Stack direction="row" spacing={2} alignItems="center" mb={2}>
            <DescriptionIcon fontSize="large" />
          ...
          <Chip
            label={project.status.toUpperCase()}
            color={isApproved ? "success" : isRejected ? "error" : isAssigned ? "info" : "warning"}
            size="large"
        ...
        <Box sx={{ p: 4 }}>
          ...
          <Paper
            variant="outlined"
            sx={{ p: 4, bgcolor: "#f9fcff", whiteSpace: "pre-line", lineHeight: 1.8 }}
          >
            {project.sow || "No content available"}
          </Paper>
          ...
          {isPending && (
            <Stack direction={{ xs: "column", sm: "row" }} spacing={3} justifyContent="center" sx={{ mt: 6 }}>
              <Button
                ...
                size="large"
                startIcon={<CheckCircleIcon />}
                onClick={() => window.confirm("Approve this SOW?") && updateStatusMutation.mutate("approved")}
                disabled={updateStatusMutation.isPending}
                sx={{ minWidth: 200 }}
              >
                Approve SOW
              ...
              <Button
                ...
                size="large"
                startIcon={<CancelIcon />}
                onClick={() => window.confirm("Reject this SOW?") && updateStatusMutation.mutate("rejected")}
                disabled={updateStatusMutation.isPending}
                sx={{ minWidth: 200 }}
              >
                Reject SOW
